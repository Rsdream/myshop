											...
											<tbody>
												...
												<tr class="cart_item">
													<td class="product-remove">
														<a href="javascript:;" class="remove"  title="删除此商品"><i class="fa fa-times" aria-hidden="true"></i></a>					
													</td>
													...
													<td class="product-price" data-title="Price">
														<span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">￥</span id="price">{{ $v['price'] }}</span>					
													</td>
													
													<td class="product-quantity" data-title="Quantity">
														<div class="quantity">
															<input type="number" step="1" min="1" max="{{ $v['stock'] }}" name="number" value="{{ $v['num'] }}" gid="{{ $v['id'] }}" title="Qty"   class="input-text qty text" size="4" pattern="[0-9]*" inputmode="numeric">
														</div>
														</div>
													</td>
													
													<td class="product-subtotal" data-title="Total">
														￥<span class="woocommerce-Price-amount amount allprice" id="price{{$v['id']}}">
														{{ $v['num']*$v['price'] }}</span>					
													</td>
										...
										@endforeach		
												...
												<tr>
													<td colspan="6" class="actions">
														...
														<div class="coupon" style="float:right;">
															<label for="coupon_code">Coupon:</label> 
															<span style="font-size: 20px">总价：<span style="color:red">￥</span><span style="color:red" id="total">{{ $total }}</span></span>
														</div>
       ...
       $('input[name="number"]').on('change', function() {
           ...
           var str = 0;
       	    	...
       	    	success:function(data) {
       	    		 $('#price'+id).html(data['price']*data['num']);       	        

       	    		 $('.allprice').each(function(index, item){
       	    		 	str += parseInt($(item).html())
       	    		 });
